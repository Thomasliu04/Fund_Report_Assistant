...
def main(argv: list[str] | None = None) -> int:
    ...
    # —— 日常主路径 ——
    serve = sub.add_parser("serve", help="【推荐】启动网页操作台")
    ...
    tables = sub.add_parser("export-tables", help="以底稿终表为基准导出表图 Excel（主产物）")
    tables.add_argument("--xlsx", required=True, help="底稿 xlsx（含终表）")
    tables.add_argument("--output", required=True, help="输出 xx_tables.xlsx")
    tables.add_argument("--period", default="", help="期别标签，如 25Q4 / 26H1")
    tables.add_argument("--focus", default="示例", help="高亮公司简称")
    tables.add_argument(
        "--force",
        action="store_true",
    ...
    imp = sub.add_parser("import-xlsx", help="从底稿 xlsx 导入 fund_metrics.csv（辅路径）")
    ...
    # —— 遗留 / 调试（正式出报不必用）——
    gen = sub.add_parser("generate", help="【遗留】生成 HTML 简报")
    ...
    p1 = sub.add_parser("phase1", help="【遗留】Phase1：灌入 PPT 第1-2页")
    ...
    loop = sub.add_parser("excel-loop", help="【遗留】最小闭环：Excel+表图 → PPT第2页")
